...
def call():
        ...
        my_w.geometry("950x350")
        Label(my_w,text="displaying data from table                                      ",font="STENCIL 30",bg="black",fg="white").place(x=0,y=225)
        Label(my_w,text="Military WTS                                                                       ",font="STENCIL 30",bg="black",fg="white").place(x=0,y=275)
        ...
        ####### end of connection ####
        my_conn.execute("SELECT * FROM m_wts")
        my=("SNO","SERVICE NO","WTS","WTS ID","DT OUT ","DT IN","REASON","BASE LOCATION")
        c = Entry(my_w, width=14, fg='white',bg='black')
        c.grid(row=0, column=7,padx=5,pady=0,ipadx=10) 
        c.insert(END, my[7])
        b=1
        i=5
